...
import static org.xbib.elasticsearch.knapsack.KnapsackHelper.*;

public class TransportBuildSuggestAction extends TransportAction<BuildSuggestRequest, BuildSuggestResponse> {
    ...
    @Inject
    public TransportBuildSuggestAction(Settings settings,
                                       ThreadPool threadPool,
                                       Client client, NodeService nodeService, ActionFilters actionFilters,
                                       IndexNameExpressionResolver indexNameExpressionResolver,
                                       KnapsackService knapsack) {
        super(settings, BuildSuggestAction.NAME, threadPool, actionFilters, indexNameExpressionResolver);
        this.client = client;
    ...
    @Override
    protected void doExecute(final BuildSuggestRequest request, ActionListener<BuildSuggestResponse> listener) {
        final KnapsackState state = new KnapsackState()
                .setMode("export")
                .setNodeName(nodeService.nodeName());
        final BuildSuggestResponse response = new BuildSuggestResponse()
                .setState(state);
        try {
     ...
     */
    final void performExport(final BuildSuggestRequest request,
        ...
        try {
            ...
            if (searchRequest == null) {
                searchRequest = new SearchRequestBuilder(client, SearchAction.INSTANCE)
                        .setQuery(QueryBuilders.matchAllQuery()).setSize(0).addAggregation(AggregationBuilders.terms(SUGGEST).field("text_suggest").size(100)).request();
            }
            for (String index : indices.keySet()) {
                searchRequest.searchType(SearchType.DEFAULT).scroll(request.getTimeout());
                if (!"_all".equals(index)) {
                ...
                Terms terms = searchResponse.getAggregations().get(SUGGEST);
